...
def create_maps(dfs_obj, configs_obj, map_type:str, show:bool):
    ...
    if map_type.upper() in('FOLIUM','ALL'):
        # Load map centred
        toronto_map = folium.Map(location=[dfs_obj.gpdf_fact_air_data_proj['latitude'].mean(), dfs_obj.gpdf_fact_air_data_proj['longitude'].mean()], zoom_start=10, control_scale=True)
        marker_cluster = MarkerCluster()

        fg = folium.FeatureGroup(name='Air Quality Measures')
        for index, row in dfs_obj.gpdf_fact_air_data_proj.iterrows():
        ...
        for index, row in dfs_obj.df_fact_traffic_volume.iterrows():
            folium.Marker(
        ...
        for index, row in dfs_obj.df_fact_gta_traffic_arcgis.iterrows():
            folium.Marker(
                location=[row['latitude'], row['longitude']],
        ...
        HeatMap(dfs_obj.df_fact_combined_air_data[['latitude','longitude','air_quality_value']],
                min_opacity=0.4,
        ...
        HeatMap(data=zip(dfs_obj.df_fact_gta_traffic_arcgis['latitude'], dfs_obj.df_fact_gta_traffic_arcgis['longitude'],
                              dfs_obj.df_fact_gta_traffic_arcgis['f8hr_pedestrian_volume']),
                min_opacity=0.4,
                blur=18).add_to(folium.FeatureGroup(name='Pedestrian Volume').add_to(toronto_map))


        HeatMap(data=zip(dfs_obj.temp_df['lat'], dfs_obj.temp_df.lng, dfs_obj.temp_df['px']),
                min_opacity=0.4,
                blur=18).add_to(folium.FeatureGroup(name='Vehicle Volume').add_to(toronto_map))

        HeatMapWithTime(dfs_obj.data, index=dfs_obj.data, auto_play=True).add_to(folium.FeatureGroup(name='Traffic Time Heatmap')).add_to(toronto_map)
    ...
    if map_type.upper() in('MAPBOX', 'ALL'):
        ...
        fig_air_quality_values = px.scatter_mapbox(dfs_obj.gpdf_fact_air_data_proj,
                             lat=dfs_obj.gpdf_fact_air_data_proj.geom.y,
                             lon=dfs_obj.gpdf_fact_air_data_proj.geom.x,
        ...
        fig_vehicle_heatmap = px.density_mapbox(dfs_obj.gpdf_fact_gta_traffic_proj,
                            lat=dfs_obj.gpdf_fact_gta_traffic_proj.geom.y,
                            lon=dfs_obj.gpdf_fact_gta_traffic_proj.geom.x,
        ...
        fig_pedestrian_heatmap = px.density_mapbox(dfs_obj.gpdf_fact_gta_traffic_proj,
                                                lat=dfs_obj.gpdf_fact_gta_traffic_proj.geom.y,
                                                lon=dfs_obj.gpdf_fact_gta_traffic_proj.geom.x,
        ...
        fig_traffic_volume = px.scatter_mapbox(dfs_obj.gpdf_fact_gta_traffic_proj.dropna(),
                                lat=dfs_obj.gpdf_fact_gta_traffic_proj.dropna().geom.y,
                                lon=dfs_obj.gpdf_fact_gta_traffic_proj.dropna().geom.x,
