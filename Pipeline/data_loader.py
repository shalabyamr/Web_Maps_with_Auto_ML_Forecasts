...
def create_staging_tables(sqlalchemy_engine):
    ...
    monthly_date_step = data_extractor.extract_monthly_data(sqlalchemy_engine=sqlalchemy_engine)
    master_list.append(monthly_date_step)

    # to execute loading the monthly forecasts into the staging layer
    monthly_forecasts_step = data_extractor.extract_monthly_forecasts(sqlalchemy_engine=sqlalchemy_engine)
    ...
    geo_names_step = data_extractor.extract_geo_names_data(sqlalchemy_engine=sqlalchemy_engine)
    master_list.append(geo_names_step)


    # to execute loading the loading ArcGIS Toronto and Peel Traffic into the staging layer
    traffic_arcgis_step = data_extractor.extract_gta_traffic_arcgis(sqlalchemy_engine=sqlalchemy_engine)
...
def create_production_tables():
    ...
    sql_files = glob.glob(configs_obj.parent_dir + '/SQL/*.sql')
    ...
    for file in sql_files:
        if 'postgis' in file:
            ...
            postgis_proj_list = create_postgis_proj_tables(configs_obj.sqlalchemy_engine, configs_obj.pg_engine)
    ...
    if configs_obj.save_locally:
        ...
        for public_table in public_tables:
            ...
            filename = configs_obj.parent_dir+'/Data/'+'Public_'+public_table+'.csv'
